...
contract RevaToken is BEP20('Reva Token', 'REVA') {
    ...
    uint public _maxSupply = 1818181818 * 1e18;

    // @dev Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public {
        require(totalSupply() + _amount <= _maxSupply, "max supply");
        require(isMinter[msg.sender], "not minter");
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    function setTreasury(address _treasury, uint _treasuryFee) public onlyOwner {
        require(_treasuryFee <= MAX_FEE, "MAX_FEE");
        treasury = _treasury;
        treasuryFee = _treasuryFee;
    }

    function setRevaStakeFeeReceiver(address _revaStakeFeeReceiver, uint _revaStakeFee) public onlyOwner {
        require(_revaStakeFee <= MAX_FEE, "MAX_FEE");
        revaStakeFeeReceiver = _revaStakeFeeReceiver;
        revaStakeFee = _revaStakeFee;
    }

    function setRevaLpStakeFeeReceiver(address _revaLpStakeFeeReceiver, uint _revaLpStakeFee) public onlyOwner {
